...
} from '@/lib/visual-match/threshold';
...
function isValidDataUri(dataUri: string): boolean {
  ...
  return /^data:image\/(png|jpeg|jpg|webp);base64,[A-Za-z0-9+/=]+$/.test(dataUri);
}
...
/** Backend threshold wins; fall back to the documented default. */
function thresholdFrom(response: { threshold: number } | null): number {
  if (response && Number.isFinite(response.threshold) && response.threshold > 0 && response.threshold < 1) {
    return response.threshold;
  }
  return VISUAL_MATCH_THRESHOLD;
}

function noMatch(threshold: number): VisualMatchOutcome {
  return { status: 'no-match', confidence: 0, threshold, candidates: [] };
}

/**
 * Runs the end-to-end visual match for one captured photo:
 *
 *   data URI → edge function (embed → vector search) → product IDs
 *     → live `products` rows (price!) → threshold decision.
 *
...
export async function matchProductFromPhoto(
  ...
  try {
    // 1. Embed + similarity search via the edge function. Invoked over
    //    HTTP so the abort/timeout signal actually reaches the request.
    ...
    if (response.candidates.length === 0) {
      return { ok: true, data: noMatch(thresholdFrom(response)) };
    }

    // 2. Fetch CURRENT product data (price included) from Supabase — the AI
    //    never supplies any of this.
    const productIds = [...new Set(response.candidates.map((candidate) => candidate.product_id))];
    ...
    // 3. Preserve SERVER order (best similarity first; the edge function
    //    already deduplicated per product) and drop ids whose product row
    //    no longer exists — deleted products must never be shown.
    const candidates: MatchCandidateView[] = response.candidates.flatMap((candidate) => {
      const product = products.get(candidate.product_id);
      return product ? [{ product, similarity: candidate.similarity }] : [];
    });

    if (candidates.length === 0) {
      // Every matched id vanished from the catalog between embedding and
      // lookup (deleted products) — report "not recognized" honestly
      // instead of showing a stale or fabricated candidate.
      return { ok: true, data: noMatch(thresholdFrom(response)) };
    }

    // 4. Threshold decision (user requirement): ≥ threshold → the matched
    //    product; < threshold → "not recognized" with candidates. The
    //    closest existing product is NOT auto-selected below threshold.
    const threshold = thresholdFrom(response);
